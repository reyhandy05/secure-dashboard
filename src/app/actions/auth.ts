...
// Menggunakan service: "gmail" agar port dan TLS disesuaikan otomatis
...
export async function sendLoginOtp(rawEmail: string) {
  ...
  if (!rateLimit(`login-otp:${hashInviteToken(email)}`, 50, OTP_TTL_MS).allowed) {
    return { success: false as const, error: "Terlalu banyak permintaan kode. Coba lagi dalam 10 menit." };
  }
  ...
  await prisma.user.update({
    where: { id: user.id },
    data: { loginOtpHash: hashLoginOtp(code), loginOtpExpires: expiresAt },
  });

  try {
    await transporter.sendMail({
      from: `"Northstar Security" <${process.env.GMAIL_USER}>`,
      to: user.email,
